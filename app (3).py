        ...
        with st.form("perfil_form"):
            ...
            with col_t1:
                r_10 = st.radio(
                    ...
                    ["Vendo todo inmediatamente", "Me preocupo pero no hago nada", "Espero y monitoreo", "Compro más (oportunidad)"],
                    key="r_10"
                )
                r_emocional = st.radio(
                    "Invertiste el equivalente a 3 sueldos. En 2 semanas perdieron un **25%** de su valor. ¿Cómo te sentís?",
                    ["Muy mal, necesito recuperar ese dinero ya", "Preocupado/a pero puedo esperar", "Incómodo/a pero confío en el largo plazo", "Tranquilo/a, es parte del juego"],
                    key="r_emocional"
                )
                r_30 = st.radio(
                    "Si baja **30%**, ¿qué hacés?",
                    ["Vendo todo de inmediato", "Vendo la mitad", "Mantengo y espero recuperación", "Compro más agresivamente"],
                    key="r_30"
                )
            ...
            with col_t2:
                r_pref = st.radio(
                    ...
                    ["No perder dinero bajo ninguna circunstancia", "Un poco de crecimiento con mínima volatilidad", "Balance entre estabilidad y rendimiento", "Maximizar rendimiento aunque haya caídas fuertes"],
                    key="r_pref"
                )
                r_crisis = st.radio(
                    "¿Cómo reaccionaste en crisis financieras anteriores?",
                    ["Vendí y salí del mercado", "Me angustié mucho pero no hice nada", "Lo tomé con calma", "Aproveché para comprar más", "No tenía inversiones aún"],
                    key="r_crisis"
                )

            _t_10      = {"Vendo todo inmediatamente": 0, "Me preocupo pero no hago nada": 33, "Espero y monitoreo": 67, "Compro más (oportunidad)": 100}
            _t_emoc    = {"Muy mal, necesito recuperar ese dinero ya": 0, "Preocupado/a pero puedo esperar": 30, "Incómodo/a pero confío en el largo plazo": 65, "Tranquilo/a, es parte del juego": 100}
            _t_30      = {"Vendo todo de inmediato": 0, "Vendo la mitad": 20, "Mantengo y espero recuperación": 60, "Compro más agresivamente": 100}
            _t_pref    = {"No perder dinero bajo ninguna circunstancia": 0, "Un poco de crecimiento con mínima volatilidad": 30, "Balance entre estabilidad y rendimiento": 60, "Maximizar rendimiento aunque haya caídas fuertes": 100}
            _t_cris    = {"Vendí y salí del mercado": 0, "Me angustié mucho pero no hice nada": 25, "Lo tomé con calma": 60, "Aproveché para comprar más": 100, "No tenía inversiones aún": 50}
            ...
            r_horizonte = st.select_slider(
                "¿En cuánto tiempo pensás usar ese capital?",
                options=["Menos de 1 año", "1 a 3 años", "3 a 5 años", "5 a 10 años", "Más de 10 años"],
                value="3 a 5 años",
                key="r_horizonte"
            )
            r_liquidez = st.radio(
                "Si en 2 años necesitaras plata urgente, ¿podrías conseguirla sin tocar estas inversiones?",
                ["No, este sería mi único colchón", "Probablemente no", "Sí, tengo otros ahorros", "Sí, tengo ingresos suficientes"],
                key="r_liquidez"
            )
            _h_score     = {"Menos de 1 año": 5, "1 a 3 años": 25, "3 a 5 años": 55, "5 a 10 años": 80, "Más de 10 años": 100}
            _liq_score   = {"No, este sería mi único colchón": -15, "Probablemente no": -5, "Sí, tengo otros ahorros": 0, "Sí, tengo ingresos suficientes": 10}
            ...
            st.caption("Respondé las preguntas. No hay problema si no sabés — el test es para conocerte mejor.")

            r_inflacion_q = st.radio(
                "¿Qué efecto tiene la inflación sobre el dinero guardado en efectivo?",
                ["Lo protege porque el efectivo es seguro", "Lo erosiona: cada año comprás menos con la misma plata", "No tiene ningún efecto", "Depende del banco"],
                key="r_inflacion_q"
            )
            r_uva_q = st.radio(
                "¿Qué es un plazo fijo UVA?",
                ["Un depósito que ajusta por inflación", "Una cuenta bancaria sin restricciones", "Un bono del Estado nacional", "Un fondo de inversión en dólares"],
                key="r_uva_q"
            )
            r_diversif_q = st.radio(
                "¿Para qué sirve diversificar una cartera de inversión?",
                ["Para garantizar ganancias siempre", "Para reducir el riesgo distribuyendo en distintos activos", "Para pagar menos impuestos", "Para invertir solo en dólares"],
                key="r_diversif_q"
            )

            _resp_correctas = {
                "r_inflacion_q": "Lo erosiona: cada año comprás menos con la misma plata",
                "r_uva_q":       "Un depósito que ajusta por inflación",
                "r_diversif_q":  "Para reducir el riesgo distribuyendo en distintos activos",
            }
            _aciertos = sum([
