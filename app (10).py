            ...
            with cols[c]:
                with st.container(border=True):
                    ...
                    with st.expander("💡 ¿Qué pasaría si invertís esta plata?", expanded=False):
                        ...
                        st.caption(f"Con tu cuota actual de **{fmt(_cuota_real, _m_meta)}** por mes, y el objetivo ajustado por inflación ({_infl_anual:.0f}% anual):")
                        ...
                        def _card(emoji, titulo, valor, descripcion, color="#888", delta_txt=None):
                            delta_html = ""
                            if delta_txt:
                                delta_html = f'<div style="font-size:0.78rem; color:#27ae60; margin-top:0.15rem;">↓ {delta_txt}</div>'
                            return f"""
                        ...
                        st.html(
                            _card("🏦", "Ahorrando (sin rendimiento)", _fmt_meses(_m_aho),
                                  "Sumás tu cuota cada mes. El objetivo crece con la inflación pero tu plata no rinde.",
                                  color="#f39c12") +
                            _card("📈", f"Invirtiendo · perfil {_perfil_inv}", _fmt_meses(_m_inv),
                                  ...
                                  color="#27ae60", delta_txt=_delta_inv)
                        )
