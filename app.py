...
with st.sidebar:
    ...
    with st.expander("Supuestos macro", expanded=False):
        ...
        st.divider()
        st.markdown("**📈 Tasas de rendimiento por perfil**")
        st.caption("Estas tasas se usan en la comparativa de cada meta para mostrar cuánto más rápido podrías llegar invirtiendo según tu perfil.")

        # Tasas por perfil — defaults asociados al risk score
        _tasas_default = {
            "Muy Conservador":   {"label": "Muy Conservador (ej: plazo fijo UVA)",     "default": 80.0},
            "Conservador":       {"label": "Conservador (ej: FCI renta fija, Lecap)",  "default": 90.0},
            "Moderado":          {"label": "Moderado (ej: FCI mixto, bonos CER)",       "default": 110.0},
            "Moderado Agresivo": {"label": "Moderado Agresivo (ej: CEDEARs, ETFs)",    "default": 140.0},
            "Agresivo":          {"label": "Agresivo (ej: acciones, renta variable)",   "default": 180.0},
        }
            ...
            with cols[c]:
                with st.container(border=True):
                    ...
                    with st.expander("💡 ¿Qué pasaría si invertís esta plata?", expanded=False):
                        _n          = int(o.get("Plazo (Meses)", 12))
                        _faltante   = float(o["faltante_futuro"])
                        ...
                        _cuota_simple = _faltante / _n if _n > 0 else _faltante
                        ...
                        if _n > 0 and _tasa_inv_m > 0:
                            _cuota_inv = _faltante * _tasa_inv_m / ((1 + _tasa_inv_m) ** _n - 1)
                        else:
                            _cuota_inv = _cuota_simple

                        _cuota_real = float(o["cuota_asignada_meta"])

                        def _meses_efectivo(faltante, cuota, infl_m):
                            if cuota <= 0: return None
                            acum, meses, costo = 0.0, 0, faltante
                        ...
                        def _meses_inversion(faltante, cuota, tasa_m):
                            if cuota <= 0: return None
                            acum, meses = 0.0, 0
                            while acum < faltante and meses < 600:
                                acum = acum * (1 + tasa_m) + cuota
                                meses += 1
                            return meses if acum >= faltante else None

                        _m_efec = _meses_efectivo(_faltante, _cuota_real, _infl_m)
                        _m_aho  = meses_para_acumular(_faltante, _cuota_real, 0.0)
                        _m_inv  = _meses_inversion(_faltante, _cuota_real, _tasa_inv_m)
                        ...
                        st.caption(f"Con tu cuota actual de **{fmt(_cuota_real, _m_meta)}** por mes:")

                        _c1, _c2, _c3 = st.columns(3)
                        _c1.metric(
                            "💸 En efectivo",
                            _fmt_meses(_m_efec),
                            help="Guardás la plata en efectivo o en una caja. La inflación la va licuando — cada mes el objetivo cuesta más.",
                        )
                        _c2.metric(
                            "🏦 Ahorrando",
                            _fmt_meses(_m_aho),
                            delta=f"{(_m_aho or 0) - (_m_efec or 0):+d} meses vs efectivo" if _m_efec and _m_aho else None,
                            delta_color="inverse",
                            help="Guardás la plata en una cuenta que al menos mantiene su valor (caja de ahorro remunerada, cuenta fintech).",
                        )
                        _c3.metric(
                            f"📈 Invirtiendo",
                            _fmt_meses(_m_inv),
                            delta=f"{(_m_inv or 0) - (_m_aho or 0):+d} meses vs ahorro" if _m_aho and _m_inv else None,
                            delta_color="inverse",
                            help=f"Invertís con una tasa anual del {_tasa_inv_anual:.0f}%, acorde a tu perfil {_perfil_inv}.",
                        )
                        ...
                        st.caption(
                            f"Tasa usada: **{_tasa_inv_anual:.0f}% anual** · perfil **{_perfil_inv}** · "
                            f"Instrumentos sugeridos: {_instrumentos_por_perfil.get(_perfil_inv, '-')}. "
                            f"Podés cambiar las tasas en **Configuración → Supuestos macro**."
                        )
