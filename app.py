            ...
            with cols[c]:
                with st.container(border=True):
                    ...
                    with st.expander("💡 ¿Qué pasaría si invertís esta plata?", expanded=False):
                        ...
                        def _card(emoji, titulo, valor, descripcion, color="#888", delta_txt=None, delta_pos=True):
                            delta_html = ""
                            if delta_txt:
                                delta_color = "#27ae60" if delta_pos else "#e74c3c"
                                delta_arrow = "↓" if delta_pos else "↑"
                                delta_html = f'<div style="font-size:0.78rem; color:{delta_color}; margin-top:0.15rem;">{delta_arrow} {delta_txt}</div>'
                            return f"""
                            <div style="border:1px solid var(--rule); border-left: 3px solid {color};
                                        border-radius:4px; padding:0.9rem 1rem; margin-bottom:0.6rem;
                              ...
                              <div style="font-family:'Fraunces',serif; font-size:1.6rem; font-weight:600;
                                          color:{color}; margin-top:0.2rem; letter-spacing:-0.02em;">
                                {valor}
                              </div>
                              {delta_html}
                        ...
                        _delta_aho = f"{abs((_m_aho or 0) - (_m_efec or 0))} meses menos que en efectivo" if _m_efec and _m_aho else None
                        _delta_inv = f"{abs((_m_inv or 0) - (_m_aho or 0))} meses menos que ahorrando" if _m_aho and _m_inv and _m_inv < _m_aho else None

                        st.html(
                            _card("💸", "Guardando en efectivo", _fmt_meses(_m_efec),
                                  "La inflación licúa tu plata: cada mes el objetivo cuesta más.", color="#e74c3c") +
                            _card("🏦", "Ahorrando (sin rendimiento)", _fmt_meses(_m_aho),
                                  "Tu plata mantiene su valor en una cuenta remunerada o fintech.", color="#f39c12",
                                  delta_txt=_delta_aho, delta_pos=True) +
                            _card("📈", f"Invirtiendo · perfil {_perfil_inv}", _fmt_meses(_m_inv),
                                  f"Tasa: {_tasa_inv_anual:.0f}% anual · {_instrumentos_por_perfil.get(_perfil_inv, '-')}",
                                  color="#27ae60", delta_txt=_delta_inv, delta_pos=True)
                        )
